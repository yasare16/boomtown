...
To begin, after **cloning** the project, run `npm i` at document root to install the very few dependencies this project 
requires.
Run `npm run start` to get going.
...
After the initial fetch call, I bunched up the other calls in a `Promise.allSettled()`. 
This allowed me to gather the resulting data and distribute content
it to the Tab component, through an array of objects. 
The odd diagram below tries to illustrate the very straight-forward architecture 
of the app based on how data travels through it:
                    ...
                    Repo   Tabs
                            |
                Events, Issues, Hooks, Members – NotFound
